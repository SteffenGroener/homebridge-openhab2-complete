...
} = require("../accessory/characteristic/Battery");
...
class Accessory {
  ...
  identify(callback) {
    this._log.debug(
      `Identify request received for: ${JSON.stringify(this._config)}`,
    );
    callback();
  }
  ...
  getServices() {
    this._log.debug(
      `Getting services for ${this.name} (${this._services.length} service(s) registered for this accessory`,
    );
  ...
  _subscribeCharacteristic(characteristic, item, transformation, callback) {
    ...
    this._openHAB.subscribe(
      item,
      function (value, habItem) {
        if (value instanceof Error) {
          this._log.error(`Error subscribing for ${item}: ${value.message}`);
        } else {
          this._log.info(
            `Received push with new state for item ${habItem}: ${value}`,
          );
          ...
          if (transformedValue !== null) {
            ...
            this._log.info(
              `Setting new transformed state for item ${habItem}: ${transformedValue}`,
            );
  ...
  _getAndCheckItemType(key, expectedItems, optional) {
    if (!this._config[key]) {
      if (optional) {
        this._log.debug(
          `${key} for ${this.name} not defined in config: ${JSON.stringify(this._config)})`,
        );
      } else {
        throw new Error(
          `Required ${key} for ${this.name} not defined: ${JSON.stringify(this._config)}`,
        );
      }
    } else {
      ...
      if (!type || type instanceof Error) {
        if (optional) {
          this._log.debug(
            `Not adding ${key} to ${this.name}: ${type ? type.message : "Item type was not synced initially and is therefore not available"}`,
          );
        } else {
          throw type
            ? type
            : new Error(
                "Item type was not synced initially and is therefore not available",
              );
        }
      } else if (expectedItems.indexOf(type) === -1) {
        if (optional) {
          this._log.debug(
            `Not adding ${key} to ${this.name}: ${item}'s type (${type}) is not as expected (${JSON.stringify(expectedItems)})`,
          );
        } else {
          throw new Error(
            `${item}'s type (${type}) is not as expected (${JSON.stringify(expectedItems)})`,
          );
        }
      } else {
        return [item, type];
  ...
  _checkInvertedConf(key) {
    if (
      this._config[key] &&
      (this._config[key] === "false" || this._config[key] === "true")
    ) {
    ...
    if (itemType === "Number" || itemType === "Rollershutter") {
      if (this._config[key]) {
        ...
        if (!isNaN(parsedValue)) {
          ...
          this._log.debug(
            `Not parsing multiplier for ${this.name}, because value (${this._config[key]}) is not parsable as float: Result ${parsedValue})`,
          );
        }
      } else {
        this._log.debug(
          `Not parsing multiplier for ${this.name}, because ${key} is not defined in config: ${JSON.stringify(this._config)}`,
        );
      }
    } else {
      this._log.debug(
        `Not parsing multiplier for ${this.name}, because of item's type ${itemType}`,
      );
    }
    return 1;
  }
  ...
  _tryBatteryService() {
    ...
    let batteryService = new this.Service.Battery(`${this.name} Battery`);
    try {
      ...
      this._log.debug(
        `Not configuring battery service for ${this.name}: ${e.message}`,
      );
    }
    return null;
  }
  ...
  getAccessory() {
    this._log.debug(
      `Returning HAP Platform Accessory for ${this.name} (${this.uuid_base})`,
    );
    let accessory = new this.API.platformAccessory(this.name, this.uuid_base);

    let defaultAccessoryInformation = accessory.getService(
